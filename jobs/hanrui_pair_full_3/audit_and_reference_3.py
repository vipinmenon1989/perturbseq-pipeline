#!/usr/bin/env python
"""Iteration 3, step 1 (SLURM only; derived from the iteration-2 audit script): full audit of the Hanrui Fang dataset with the NEWLY supplied
...
def main() -> int:
    ...
    ap.add_argument("--prev-guide-ref", required=True, help="iteration-1 reference with read-derived scaffold classes per design guide (empirical scaffold comparison)")
    ap.add_argument("--prev-counts", required=True, help="iteration-2 result dir whose scaffold-specific guide counts are re-used (copied with md5 verification)")
    ap.add_argument("--manifest-name", default="Hanrui_Fang_pair_guide_full_3_sample_manifest.csv")
    a = ap.parse_args()
    ...
    prev_ref = pd.read_csv(a.prev_guide_ref, dtype=str, keep_default_na=False).set_index("guide_id")
    prev_audit_ref = None
    g_rows = []
    ...
    fact("F13", "Were non-cloned designed spacers observed in reads?", a.prev_guide_ref, "umis_in_cells_all_wells",
    ...
    man_path = repo / "config" / a.manifest_name
    ...
    # ============================================================================================
    cmp_rows = [
        {"aspect": "experimental documents", "previous_run_full_2": "design_out_v2.xlsx + 20250822_OligoPool_TwistAddG.xlsx + Scaffold_seq.docx", "new_run_full_3": "identical files (no new dataset file since 2026-09-15 15:19; md5 recorded in source_file_inventory.csv)", "changed": "no"},
        {"aspect": "scaffold interpretation", "previous_run_full_2": "A = document Scaffold 2 = construct position 2; C = document Scaffold 1 = construct position 1", "new_run_full_3": "same (re-derived from the oligo backbone / BlpI columns and read consensus)", "changed": "no"},
        {"aspect": "pair mapping", "previous_run_full_2": f"explicit construct map, {len(constructs)} constructs, ';'-joined ids for multi-construct slots", "new_run_full_3": "same map, same feature ids", "changed": "no"},
        {"aspect": "valid control definition", "previous_run_full_2": f"{n_ntc_constructs} designed NO-TARGET_<n>F constructs (pair_non_targeting)", "new_run_full_3": "same", "changed": "no"},
        {"aspect": "MOI interpretation", "previous_run_full_2": "not documented; observed multiplicity reported; multi-construct cells ambiguous / unresolved", "new_run_full_3": "same", "changed": "no"},
        {"aspect": "sample / replicate labels", "previous_run_full_2": "well-id derived (condition = HF011/HF012 group, replicate = A/B); no experimental metadata file", "new_run_full_3": "same (still no document defines the wells)", "changed": "no"},
        {"aspect": "designed targeting+NTC (_S1) constructs", "previous_run_full_2": "strict primary targeting label (pooled with dual-guide constructs; strata reported)", "new_run_full_3": "SENSITIVITY stratum only: status pair_targeting_plus_ntc, class ambiguous, excluded from primary hit calls (guides.designed_targeting_plus_ntc_primary: false)", "changed": "YES"},
        {"aspect": "slot dominance rule", "previous_run_full_2": "top_umi >= 3 and top_umi > 2.0 x second_umi", "new_run_full_3": "top_umi >= 3 and (top_umi + 1) / (second_umi + 1) >= 2.0, computed and stored per scaffold slot (guide_<A|C>_dominance_ratio); identical decisions for integer counts", "changed": "formulation / provenance only"},
        {"aspect": "primary control", "previous_run_full_2": "designed NTC pairs", "new_run_full_3": "same", "changed": "no"},
        {"aspect": "guide count matrices", "previous_run_full_2": "scaffold-specific counting (SLURM 20049321)", "new_run_full_3": "same matrices, copied with md5 verification (inputs/guide_counts/PROVENANCE.md)", "changed": "no"},
        {"aspect": "measured transcript", "previous_run_full_2": "target_symbol (HGNC renames CEBPB / ILRUN / KYAT3 / ZNG1A; SNP loci -> gene)", "new_run_full_3": "same, now built into the reference at audit time", "changed": "no"},
    ]
    cmp_df = pd.DataFrame(cmp_rows)
    cmp_df.to_csv(audit / "previous_vs_new_reference_comparison.csv", index=False)
    part1 = ["# Previous (full_2) versus new (full_3) run", "", f"Part 1 (design / reference level, SLURM job {JOB}, {NOW:%Y-%m-%d %H:%M}); part 2 (run-level results) is appended by the root-report job.", "",
             f"Previous run: `{prev}` (not modified). New run: `{out}`.", "", md_table(cmp_df), "",
             "## Scaffold agreement per cloned guide (document vs iteration-1 read-derived class)", "", md_table(gaudit[gaudit.in_oligo_pool].design_vs_previous_empirical_scaffold.value_counts().rename_axis("agreement").reset_index(name="n_guides")), ""]
    (audit / "previous_vs_new_run.md").write_text("\n".join(part1) + "\n")
    ...
    # ============================================================================================
    # 10. Iteration-3 deliverables: requested file names, re-used guide counts, information record
    # ============================================================================================
    import shutil
    prev_counts = Path(a.prev_counts)
    per_well = pd.read_csv(prev_counts / "audit" / "guide_counts_per_well.csv", dtype=str, keep_default_na=False)
    allw = per_well[per_well.well == "ALL"].set_index("guide_id")
    ref = pd.read_csv(audit / "pair_guide_reference.csv", dtype=str, keep_default_na=False).set_index("guide_id")
    ref["reads_matched_all_wells"] = allw.loc[ref.index, "reads_matched_all_barcodes"].astype(int).to_numpy()
    ref["umis_in_cells_all_wells"] = allw.loc[ref.index, "umis_in_cells"].astype(int).to_numpy()
    ref["cells_ge3umi_all_wells"] = allw.loc[ref.index, "cells_positive_ge3"].astype(int).to_numpy()
    ...
    ref["sequence_match_status"] = ref["sequence_match_status"] + np.where(ref["observed_in_reads"], "; observed in reads (exact 20-mer / unambiguous 1-mismatch; counting job 20049321)", "; not observed in any well")
    ...
    ref["counting_job"] = "20049321 (iteration 2; matrices re-used, md5-verified)"
    ref.reset_index().to_csv(audit / "guide_reference_used.csv", index=False)
    (audit / "pair_guide_reference.csv").unlink()
    constructs.to_csv(audit / "construct_pair_map_used.csv", index=False)
    (audit / "pair_mapping_audit.csv").unlink()
    inv.to_csv(audit / "source_file_inventory.csv", index=False)
    (audit / "data_inventory.csv").unlink()
    manifest.to_csv(audit / "sample_manifest_used.csv", index=False)
    # guide count matrices: copy from the iteration-2 result with md5 verification
    gc_root = out / "inputs" / "guide_counts"
    gc_root.mkdir(parents=True, exist_ok=True)
    prov = [f"# Guide count matrices (iteration 3)", "", f"Copied by SLURM job {JOB} on {NOW:%Y-%m-%d} from `{prev_counts / 'inputs' / 'guide_counts'}` (scaffold-specific counting, SLURM job 20049321; see `{prev_counts / 'audit' / 'guide_quantification_methods.md'}`). "
            "The counting inputs (FASTQs, design workbook, Cell Ranger barcodes) and the counter code are unchanged since that job, so the matrices are re-used rather than recomputed. md5 of every copied file:", "", "| well | file | md5 (source) | md5 (copy) | identical |", "|---|---|---|---|---|"]
    for w in WELLS:
        (gc_root / w).mkdir(exist_ok=True)
        for f in sorted((prev_counts / "inputs" / "guide_counts" / w).iterdir()):
            if f.is_file():
                dst = gc_root / w / f.name
                shutil.copy2(f, dst)
                m1, m2 = md5(f), md5(dst)
                assert m1 == m2, f"md5 mismatch after copy: {f}"
                prov.append(f"| {w} | {f.name} | {m1} | {m2} | {m1 == m2} |")
    (gc_root / "PROVENANCE.md").write_text("\n".join(prov) + "\n")
    for f in ("guide_read_retention_funnel.csv", "guide_quantification_summary.csv", "off_design_feature_summary.csv", "guide_counts_per_well.csv", "guide_quantification_methods.md", "scaffold_read_consensus.json"):
        if (prev_counts / "audit" / f).is_file():
            shutil.copy2(prev_counts / "audit" / f, audit / f)
    # experimental information record (design record + what changed + iteration-3 rules)
    design_md = (audit / "experimental_design_audit.md").read_text()
    changed = ["", "## What the confirmed experimental information changed (iteration 3 versus iteration 2)", "",
               "| item | changed? | statement |", "|---|---|---|",
               "| scaffold interpretation | **no** | A = document Scaffold 2 = construct position 2 (anchor GTTTAAGAGCTA); C = document Scaffold 1 = position 1 (anchor GTTTCAGAGCTA); confirmed by the oligo backbone / BlpI columns and by the read consensus of all four wells |",
               f"| pair mapping | **no** | explicit construct map, {len(constructs)} constructs ({constructs.construct_type.value_counts().to_dict()}); features are scaffold-specific; a slot feature can belong to several constructs |",
               f"| valid control definition | **no** | pair_non_targeting = the {n_ntc_constructs} designed NO-TARGET_<n>F constructs only |",
               "| MOI interpretation | **no** | not documented in any file; observed multiplicity is reported; cells with several strong features per scaffold or with non-designed combinations are ambiguous / unresolved |",
               "| sample / replicate labels | **no** | no document defines HF011 vs HF012 or A vs B; labels remain well-id derived and are flagged as such in sample_manifest_used.csv |",
               "| primary cell-assignment rule | **YES** | strict primary pairs are now targeting-targeting (dual-guide) constructs and NTC-NTC constructs only; the 41 designed targeting+NTC (_S1) constructs form a labelled sensitivity stratum (status pair_targeting_plus_ntc, excluded from primary hit calls). The per-slot rule is stated explicitly as top_umi >= 3 and (top_umi + 1) / (second_umi + 1) >= 2.0, applied independently to scaffold A and C, and the ratio is stored per slot |",
               "", "## Iteration-3 assignment rules (configuration `config/Hanrui_Fang_pair_guide_full_3.yaml`)", "",
               "1. For each scaffold class the strongest feature must have top_umi >= 3 and dominance ratio (top_umi + 1) / (second_umi + 1) >= 2.0 (`guide_A_*`, `guide_C_*` obs columns store top, second, ratio, slot status).",
               "2. Both slots must pass (`incomplete_pair` otherwise; `ambiguous_scaffold_*` when a slot has several strong features).",
               "3. The two features must share a documented construct id (`construct_pair_map_used.csv`); otherwise `unresolved_pair` (detail: same_target_not_designed / targeting_plus_ntc_not_designed / ntc_pair_not_designed) or `dual_target_ambiguous`.",
               "4. Strict primary labels: designed targeting-targeting constructs -> `pair_targeting` (perturbation_class targeting); designed NTC-NTC constructs -> `pair_non_targeting` (control). Designed targeting+NTC constructs -> `pair_targeting_plus_ntc` (sensitivity stratum, class ambiguous).",
               "5. Pairs are never chosen as the two most abundant guides of the whole cell; same-target A+C combinations that are not constructs are never valid (the map fixes three pairs per target)."]
    info_md = design_md.replace("# Experimental design record: Hanrui Fang dual-guide Perturb-seq (iteration 2)", "# Experimental information used: Hanrui Fang dual-guide Perturb-seq (iteration 3)")
    info_md = info_md.replace("## Consequences for the analysis", "\n".join(changed) + "\n\n## Consequences for the analysis (iteration-2 wording, superseded by the iteration-3 rules above where they differ)")
    (audit / "experimental_information_used.md").write_text(info_md)
    (audit / "experimental_information_used.html").write_text(md_to_html(info_md, "Experimental information used (Hanrui Fang, iteration 3)"))
    for f in ("experimental_design_audit.md", "experimental_design_audit.html"):
        (audit / f).unlink()
    summary.append(f"- iteration-3 deliverables written: guide_reference_used.csv ({len(ref)} features), construct_pair_map_used.csv, source_file_inventory.csv, sample_manifest_used.csv, experimental_information_used.md/.html, previous_vs_new_run.md; guide counts copied from {prev_counts.name} with md5 verification")
