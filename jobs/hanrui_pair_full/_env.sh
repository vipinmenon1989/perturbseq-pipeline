# Shared preamble for the Hanrui Fang pair-guide full-analysis SLURM jobs.
set -euo pipefail
...
CFG=$REPO/config/Hanrui_Fang_pair_guide_full.yaml
cd "$REPO"
...
echo "HOST: $(hostname)  DATE: $(date -Is)  JOB: ${SLURM_JOB_NAME:-?}/${SLURM_JOB_ID:-none}  CPUS: ${SLURM_CPUS_PER_TASK:-?}  MEM: ${SLURM_MEM_PER_NODE:-?}"
