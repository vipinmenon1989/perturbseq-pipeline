# WIP Scientific Reading: Multi-Dimensional Phenotypic Decomposition
...
- **Strongest Global Phenotype Shifts (Energy Distance):**
  1. *PDX1het*: Energy Distance = 13.6337 (MMD = 0.3129) ($n=469$ cells)
  2. *HHEXhet*: Energy Distance = 10.4732 (MMD = 0.2307) ($n=328$ cells)
  3. *KDM2B*: Energy Distance = 9.3204 (MMD = 0.2087) ($n=182$ cells)
  4. *HHEX*: Energy Distance = 8.9955 (MMD = 0.2034) ($n=1,214$ cells)
  5. *GLIS3*: Energy Distance = 7.9693 (MMD = 0.1854) ($n=935$ cells)
  6. *GATA6*: Energy Distance = 9.7045 (MMD = 0.1590) ($n=1,500$ cells)
  7. *GSC*: Energy Distance = 5.8760 (MMD = 0.1255) ($n=1,652$ cells)
  8. *QSER1TET1*: Energy Distance = 5.6360 (MMD = 0.1262) ($n=1,157$ cells)
  9. *HHEXe*: Energy Distance = 5.6227 (MMD = 0.1227) ($n=1,487$ cells)
  10. *FOXA2*: Energy Distance = 5.4466 (MMD = 0.1063) ($n=4,896$ cells)
...
- **Extreme Focal Trapping (Peak lochNESS):**
  1. *FOXA2*: dominant celltype lochNESS = +13.740 (Peak cell lochNESS = +18.307) (dominant cell type: Liver, fraction = 34.0%)
  2. *GLIS3*: dominant celltype lochNESS = +13.625 (Peak cell lochNESS = +25.564) (dominant cell type: PDP, fraction = 64.5%)
  3. *GATA6*: dominant celltype lochNESS = +4.530 (Peak cell lochNESS = +37.059) (dominant cell type: Endothelial, fraction = 34.5%)
  4. *BMPR1A*: dominant celltype lochNESS = +4.129 (Peak cell lochNESS = +9.820) (dominant cell type: Stromal, fraction = 34.2%)
  5. *BCOR*: dominant celltype lochNESS = +3.304 (Peak cell lochNESS = +7.969) (dominant cell type: ESC, fraction = 16.3%)
  6. *ARX*: dominant celltype lochNESS = +3.084 (Peak cell lochNESS = +9.326) (dominant cell type: SC-alpha, fraction = 23.2%)
...
We performed rigorous Spearman rank ($
ho$) and Pearson ($r$) correlation analyses across all evaluated genotypes:

| Pairwise Comparison | $n$ | Spearman $
ho$ | $p$-value (Spearman) | Pearson $r$ | $p$-value (Pearson) | Biological Relationship |
|---|---|---|---|---|---|---|
...
- **The Unresolved Compression:** While pertTF *predicts* these outcomes, an end-to-end neural network compresses multiple biological phenomena into a single latent vector. It does not explicitly tell the biologist:
  ...
  - Why do some master regulators (like *PAX6*) have modest global Energy Distance (1.637, MMD 0.038) yet produce complete lineage collapse of SC-beta and dramatic SC-EC diversion?

### The Multi-Dimensional Decomposition Solution
The WIP demonstrates that perturbation phenotypes must be decomposed into **three complementary, partially coupled coordinates**:
...
Integrating these decomposed coordinates (PS, Energy Distance, signed lochNESS) into the next generation of pertTF loss functions will enable **phenotype-aware perturbation prediction models** that predict not just static expression vectors, but complete multi-dimensional phenotypic manifolds.
