#!/usr/bin/env python3
"""Generate auditable presentation_source_summary.md linking every slide to exact data artifacts."""

import os
from pathlib import Path

OUT_PATH = Path("/local/projects-t3/lilab/vmenon/PertTF-Virtual-Challeng-Weilab/perturbseq-pipeline/results/diabetes_specific/presentation/presentation_source_summary.md")

summary_content = """# Presentation Source Summary: Diabetes-Specific Perturb-seq Analysis
...
- **Exact Numerical Findings**: 313 significant associations ($\text{FDR} < 0.05$). Striking diversions: GATA6 $\rightarrow$ Endothelial ($25.7\%$ vs $0.7\%, \log_2 \text{OR} = +5.21, \text{FDR} = 1.05 \times 10^{-141}$); FOXA2 $\rightarrow$ Liver ($34.0\%$ vs $2.0\%, \log_2 \text{OR} = +4.09, \text{FDR} = 3.65 \times 10^{-224}$); NEUROG3 $\rightarrow$ complete SC-beta loss ($0.0\%$ cells, $\log_2 \text{OR} = -\infty$); GSC $\rightarrow$ Stromal ($\log_2 \text{OR} = +2.66$).
...
- **Exact Numerical Findings**: Spearman $\rho = +0.7832, p = 2.24 \times 10^{-6}, n = 26$. Top concordant hits: KDM2B, GLIS3, HHEX, GATA6.
...
- **Tables Used**: `perturbation_summary.csv`.
- **Exact Numerical Findings**: Distance vs Positive lochNESS: $\rho = +0.4103, p = 0.0129, n=36$; Distance vs Absolute lochNESS: $\rho = +0.4512, p = 0.0057, n=36$; Distance vs Negative lochNESS: $\rho = -0.1410, p = 0.4263, n=34$.
...
- **Exact Numerical Findings**: ORA against MSigDB Hallmark, Reactome, and GO BP: P2 $\rightarrow$ HALLMARK_PANCREAS_BETA_CELLS (overlap 36/42, $\text{FDR} = 1.28 \times 10^{-3}, \text{OR} = 4.97$); P3 $\rightarrow$ HALLMARK_MYC_TARGETS_V1 (overlap 76/78, $\text{FDR} = 3.50 \times 10^{-36}, \text{OR} = 104.8$), MTORC1_SIGNALING ($\text{FDR} = 3.05 \times 10^{-20}$), TRANSLATION ($\text{FDR} = 1.13 \times 10^{-19}$); P4 $\rightarrow$ EPITHELIAL_MESENCHYMAL_TRANSITION (overlap 14/36, $\text{FDR} = 1.47 \times 10^{-4}, \text{OR} = 5.69$), ANGIOGENESIS ($\text{FDR} = 3.12 \times 10^{-4}$).
...
- **Exact Numerical Findings**: HNF4A ($n=987, E=1.75$) vs HNF4Ahet ($n=829, E=1.86$) $\rightarrow d = 0.0905$ (Rank 1 closest pair, PG3, M1). GLIS3 ($n=935$) $\rightarrow$ PS median $= 0.7500$ (Rank 1), 66.6% responders, $E = 8.21$, positive lochNESS $= +9.53$ (Rank 4, PG2, M1).
...
print(f"presentation_source_summary.md written successfully to: {OUT_PATH}")
